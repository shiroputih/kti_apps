...
?>
            ...
            <div id="tablekti" class="table-responsive">
                <table class="table table-hover" id="dataTable" width="100%" cellspacing="0">
                    ...
                    <form method="POST">
                        <?php
                        $sql = "SELECT kti.nim, mahasiswa.nama, kti.judul_penelitian,kti.file_kti,kumpul_berkas.tgl_kumpulberkas, batas_waktu.batas_kumpulberkas,d1.id_dosen as dosen1, d2.id_dosen as dosen2,d3.id_dosen as dosen3,n1.status_nilai as n1,n2.status_nilai as n2,n3.status_nilai as n3,nilai_angka_final,nilai_huruf_blmkumpul,nilai_huruf
                        FROM kti
                        ...
                        JOIN batas_waktu ON batas_waktu.nim = kti.nim
                        JOIN nilai_akhir ON nilai_akhir.nim = kti.nim
                        ...
                        if ($result->num_rows > 0) {
                            while ($row = $result->fetch_assoc()) {
                                echo "<tr>
                                        ...
                                        <td>$row[nilai_huruf_blmkumpul]</td>
                                        <td>$row[nilai_huruf]</td>

                                        <td>";
                                        if($row['n1']=='1' && $row['n2']=='1' && $row['n3']=='1'){
                                            echo"
                                            <a id ='finalisasinilai' data-nim=$row[nim] data-nama='$row[nama]' data-judulkti='$row[judul_penelitian]' data-iddosen1 ='$row[dosen1]' data-filekti=$row[file_kti]
                                            data-toggle='modal' data-target='#FinalisasiNilaiModal'>
                                            <button type='submit' class='btn btn-danger btn-sm' ><img src='icons/check.png' width='20px' height='20px'></button></a>
                                            ";
                                        }
                                        echo"
                                        </td>
